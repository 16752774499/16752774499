...
case 'tuandui':
if(!$conf['jiangli'])exit('{"code":-1,"msg":"当前站点未开启个人奖励"}');
...
if ($DB->getRow("SELECT * FROM pre_qiandao66 WHERE zid='{$userrow['zid']}' AND date='$day' AND reward=$ggu ORDER BY id DESC LIMIT 1")) {
    exit('{"code":-1,"msg":"奖励已经领取！"}');
} else {
    // 如果奖励尚未领取，则继续执行签到流程
    $claimRewardBtn1 = $DB->getColumn("SELECT count(*) FROM qq pre_qiandao66 WHERE reward=$ggu"); 
    if ($claimRewardBtn1 >= 1) {
        exit('{"code":-1,"msg":"已经签到！"}');
    }
    if ($DB->query("INSERT INTO `zid`,`qq`,`reward`,`date`,`time`,`continue`,`ip`) VALUES ('{$userrow['zid']}', '$day', '$reward')")) {
        exit('{"code":0,"msg":"领取成功！","reward":' . $reward . '}');
    }
}

if($continue > $conf['qiandao_day']) $continue = $conf['qiandao_day'];
$reward = $ggu;  //更改lin奖励金额为20
...
$sql="INSERT INTO `pre_qiandao66` (`zid`,`qq`,`reward`,`date`,`time`,`continue`,`ip`) VALUES ('".$userrow['zid']."','".$userrow['qq']."','".$reward."','".$day."','".$date."','".$continue."','".$clientip."')";
if($DB->exec($sql)){
    unset($_SESSION['isqiandao']);
    changeUserMoney($userrow['zid'], $reward, true, '奖励', '每日发展合伙人现金奖励');
    $result = array('code'=>0, 'msg'=>'领取成功，获得'.$reward.'元现金奖励！');
}else{
    $result = array('code'=>-1, 'msg'=>'领取失败'.$DB->error());
}
exit(json_encode($result));
break;
...
case 'tuandui2':
	if(!$conf['jiangli2'])exit('{"code":-1,"msg":"当前站点未开启个人奖励"}');
...
if ($conf['qiandao_limitip']==1 && $DB->getRow("SELECT * FROM pre_qiandao66 WHERE ip='{$clientip}' AND date='$day' ORDER BY id DESC LIMIT 1")) {
    exit('{"code":-1,"msg":"您的IP今天已经领取过了！"}');
}
if ($row = $DB->getRow("SELECT * FROM pre_qiandao66 WHERE zid='{$userrow['zid']}' AND date='$lastday' ORDER BY id DESC LIMIT 1")) {
...
case 'tuandui3':
	if(!$conf['jiangli3'])exit('{"code":-1,"msg":"当前站点未开启个人奖励"}');
...
if ($DB->getRow("SELECT * FROM pre_qiandao66 WHERE zid='{$userrow['zid']}' AND date='$day' AND reward=$ggu ORDER BY id DESC LIMIT 1")) {
    ...
    if ($DB->query("INSERT INTO `zid`,`qq`,`reward`,`date`,`time`,`continue`,`ip`) VALUES ('{$userrow['zid']}', '$day', '$reward')")) {
        exit('{"code":0,"msg":"签到成功！","reward":' . $reward . '}');
    }
}
...
if($DB->exec($sql)){
    unset($_SESSION['isqiandao']);
    changeUserMoney($userrow['zid'], $reward, true, '奖励', '个人');
    $result = array('code'=>0, 'msg'=>'领取成功，获得'.$reward.'元现金奖励！');
}else{
    $result = array('code'=>-1, 'msg'=>'领取失败'.$DB->error());
}
...
case 'qdcount':
	...
	$count4=$DB->getColumn("SELECT sum(reward) FROM pre_qiandao");
	$rewardcount=$DB->getColumn("SELECT sum(reward) FROM pre_qiandao WHERE zid='{$userrow['zid']}'");
	$result=array("count1"=>$count1,"count2"=>$count2,"count3"=>$count3,"count4"=>$count4,"rewardcount"=>round($rewardcount,2));
	exit(json_encode($result));
break;
case 'msg':
	if($userrow['power']==2){
...
case 'recharge':
	...
	$data=[':trade_no'=>$trade_no, ':tid'=>-1, ':input'=>(string)$userrow['zid'], ':name'=>'余额加款', ':money'=>$value, ':ip'=>$clientip];
	if($DB->exec($sql, $data)){
		exit('{"code":0,"msg":"提交订单成功！","trade_no":"'.$trade_no.'","money":"'.$value.'","name":"余额加款"}');
	}else{
