...
case 'payrmb':
	...
	if($srow['status']==0){
		...
		if($DB->exec("UPDATE `pre_site` SET `rmb`=`rmb`-'{$srow['money']}' WHERE `zid`='{$userrow['zid']}'") && $DB->exec("UPDATE `pre_pay` SET `type`='rmb',`status`='1',`endtime`=NOW() WHERE `trade_no`='{$orderid}'")){
			$srow['type'] = 'rmb';
			$payorder=$DB->getRow("SELECT * FROM pre_pay WHERE trade_no='{$orderid}' LIMIT 1");
	...
	case 'cidr':
	     	$cid=intval($_GET['cid']);
	     $rs=$DB->query("SELECT * FROM pre_class WHERE cidr='$cid' AND active=1");
	     $count8=$DB->getColumn("SELECT count(*) from pre_class WHERE cidr='$cid'");
	...
	exit(json_encode($result));
	    	break;
	    		case 'cidrs':
	    		    	$cid=intval($_GET['cid']);
	    		    $rs=$DB->query("SELECT * FROM pre_class WHERE cid='$cid' AND active=1");
	    		    $res = $rs->fetch();
	    		     $count8=$DB->getColumn("SELECT count(*) from pre_tools WHERE cid='$cid'");
...
case 'gettoolnew':
	...
	if($cid){
		...
		$rs1=$DB->query("SELECT * FROM pre_class WHERE cidr='$cid'");
			while($res1 = $rs1->fetch()){
			    $cc=$res1['cid'];
			     $icc1=$DB->getColumn("SELECT count(tid) FROM pre_tools WHERE cid='$cc'");
			     $ic1=$ic1+$icc1;
	   $erjiid=$res1['cid'];
	   $rs3=$DB->query("SELECT * FROM pre_tools WHERE cid='$erjiid' AND active=1 ORDER BY $orderBy LIMIT $page,$limit");
	   ...
	   while($res3 = $rs3->fetch(PDO::FETCH_ASSOC)){
		...
		if($res3['is_curl']==4){
			$isfaka = 1;
			$count = $DB->getColumn("SELECT count(*) FROM pre_faka WHERE tid='{$res3['tid']}' AND orderid=0");
...
case 'pay':
	...
	if($tool && $tool['active']==1){
		...
		if($need==0 && $tid!=$_SESSION['gift_tid']){
			...
			$thtime=date("Y-m-d").' 00:00:00';
			if($_SESSION['blockfree']==true || $DB->getColumn("SELECT count(*) FROM `pre_pay` WHERE `money`=0 AND `ip`='$clientip' AND `status`=1 AND `addtime`>'$thtime'")>=50){
...
case 'order': //订单进度查询
	...
	$spxq_xx=$DB->getRow("SELECT * FROM pre_config WHERE k='spxq_xx' LIMIT 1");
	
	$result=array('code'=>0,'msg'=>'succ','name'=>$tool['name'],'money'=>$row['money'],'date'=>$row['addtime'],'inputs'=>$inputsdata,'list'=>$list,'kminfo'=>$kmdata,'alert'=>$tool['alert'],'spxq_xx'=>$spxq_xx['v'],'desc'=>$tool['desc'],'status'=>$row['status'],'result'=>$row['result'],'complain'=>intval($conf['show_complain']),'islogin'=>$islogin2,'selfrefund'=>$conf['selfrefund']);
...
case 'gettoolday':
	...
	if($cid){
	  ...
	  if( $_SESSION['vtid']>1){
	      $tt= $_SESSION['vtid'];
	       $rs3=$DB->query("SELECT *,round(price,4) as price2 FROM pre_tools WHERE cid='$erjiid' AND tid<=$tt ORDER BY $orderBy LIMIT $page,$limit");}else{
	     $rs3=$DB->query("SELECT *,round(price,4) as price2 FROM pre_tools WHERE cid='$erjiid' ORDER BY $orderBy LIMIT $page,$limit");
	}
	...
	//tid AND tid<=1860 正序改成>   倒序代为<  
	 session_start();
	  if( $_SESSION['vtid']>1){
	      $tt= $_SESSION['vtid'];
	$rs=$DB->query("SELECT * FROM pre_tools WHERE $where AND tid<=$tt ORDER BY $orderBy LIMIT $page,$limit");}else{
	    	$rs=$DB->query("SELECT * FROM pre_tools WHERE $where ORDER BY $orderBy LIMIT $page,$limit");
	}
...
function processOrder2($srow, $is_fenzhan = true)
{
	...
	if ($srow['tid'] == -2) {
		...
		if ($type == "update") {
			...
			$sql="INSERT INTO `pre_site` (`upzid`,`power`,`domain`,`domain2`,`user`,`pwd`,`giftcode`,`imvitecode`,`rmb`,`qq`,`sitename`,`title`,`keywords`,`description`,`anounce`,`alert`,`kfqq`,`addtime`,`endtime`,`status`) VALUES (:upzid, :power, :domain, NULL, :user, :pwd, :giftcode, :invitecode, :rmb, :qq, :sitename, :title, :keywords, :description, :anounce, :alert, :kfqq, NOW(), :endtime, 1)";
...
while ($current_zid > 1 && $parent_zid == 0) {
    ...
    // 查询当前站点的信息
    $site_info = $DB->getRow("SELECT * FROM pre_site WHERE zid='" . $current_zid . "' LIMIT 1");
