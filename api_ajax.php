...
switch ($act) {
	...
	case 'gettool':
		...
		$label_arr = [];
		$sql = "SELECT t.label FROM pre_tools as t WHERE $where";
		$rs=$DB->query($sql);
		...
		$count_sql = "SELECT count(tid) FROM pre_tools as t WHERE $where";
		//echo $count_sql;die();
		//按订单销量
        //$sql = "SELECT t.*,(select count(id) from shua_orders WHERE tid = t.tid) as xiaoliang FROM pre_tools as t WHERE $where ORDER by $orderBy limit $page,$limit";

        //按人气值
        $sql = "SELECT t.*,t.renqi as xiaoliang FROM pre_tools as t WHERE $where ORDER by $orderBy limit $page,$limit";
...
file_put_contents(dirname(__FILE__)."/log.php",var_export(['0'=>$price,'1'=>$price_arr,'2'=>$xs_price],true));
...
function get_data_show(){
	...
	switch ($arr) {
		...
		case 2:
			//查询数据
			$rs = $DB->query("SELECT name FROM pre_tools where `active` = 1 and `close` = 0");
			$goods_list;
				while ($res = $rs->fetch()) {
					unset($res[0]);
				    $goods_list[] = $res;
