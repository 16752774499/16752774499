                    ...
                    success : function(res) {
                        console.log(res);
                            ...
                            layui.each(res.data, function(index, item){
                                html = '<a class="fui-goods-item" title="'+item.name+'" onclick="ifbox(\'./?mod=buy&tid='+item.tid+'\')">';
                                ...
                                //库存为0的
                                    ...
                                    html += '<div class="name" style="font-size: 13px;color: #000000;">'+item.name+'</div>';
                                    var price = '';
                                    if(item.price <=0){
                                        price = '';
                                    }
                                    html += '';
                                    if(item.price <=0){
                                        buy = '<div style="height: 2rem"><span class="buy" style="font-size: 13px;background: #yellowgreen;color:#fff;display: inline-block;height: 2rem;line-height: 0.9rem;float: right;padding: 0.05rem 0.45rem;width:100%;border-radius: 0.2rem;    border: 1px solid transparent;text-align:center;">免费领<br>限时抢购</span></div>';
                                    }else{
                                        buy = '<div style="height: 2rem"><span class="buy" style="font-size: 13px;background: #fd2411;color:#fff;display: inline-block;height: 2rem;line-height: 0.9rem;float: right;padding: 0.05rem 0.45rem;width:100%;border-radius: 0.2rem;    border: 1px solid transparent;text-align:center;"><br>正在热卖</span></div>';
                                    }

                                    if(item.stock == 0){
                                        buy = '<div style="height: 2rem"><span class="buy" style="font-size: 13px;background: #aeadad;color:#fff;display: inline-block;height: 2rem;line-height: 0.9rem;float: right;padding: 0.05rem 0.45rem;width:100%;border-radius: 0.2rem;    border: 1px solid transparent;text-align:center;">已卖光<br>暂时缺货</span></div>';
                                    }

                                    if(item.close == 1){
                                        buy = '<div style="height: 2rem"><span class="buy" style="font-size: 13px;background: #aeadad;color:#fff;display: inline-block;height: 2rem;line-height: 0.9rem;float: right;padding: 0.05rem 0.45rem;width:100%;border-radius: 0.2rem;    border: 1px solid transparent;text-align:center;">已下架<br>暂停售卖</span></div>';
                                    }

                                    html += '<div class="price" style="margin-top: 0.2rem;background-color:rgba(253,233,170);height: 2rem;border-radius: 0.2rem; "><span class="text" style="color:#974b01;font-size:0.65rem;line-height:2rem;padding-left: 10px">'+ kucun +'</span>'+buy+'</div>';
                                    html += '</div>';
                                    html += '</a>';
                                    lis.push(html);
                                });
                            ...
                            if(kw != ""){
                                   ...
                                   $(".catname_c").hide(); $(".catname_show").show();  $(".catname_cc").hide();
                                $(".catname_show").html('包含<font style="color: #7d7c7a;font-size: 12px;">'+kw+'</font>共有<font style="">'+res.total+'</font>件商品');
                            }else{
                                   $(".catname_c").hide(); $(".catname_show").show();  $(".catname_cc").hide();
                                   $(".catname_show").html('全站商品共'+res.count+'个,当前'+res.total+'个');                            }
                            layer.closeAll();
