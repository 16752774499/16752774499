            ...
            success: function (data) {
                if (data.code == 0) {
                    ...
                    $.each(data.data, function (key, val) {
                        ...
                        content += '<li>\n' +
                            '          <a href="' + url + '">\n' +
                            '              <img src="' + val.shopimg + '" onerror="this.src=\'assets/store/picture/error_img.png\'" style="height: ' + [$(window).width() > 460 ? '15' : '15'] + 'vh" class="proimg"/>\n' +
                            '              <p class="tit">' + val.name + '</p>\n' +
                            '              <p class="price" style="font-size: 14px;"><b>￥' + val.price + '元</b></p>\n' +
                            '          </a>\n' +
                            '      </li>';
                    });
    ...
    Success: function (data) {
        ...
        $.each(data.data, function (key, val) {
            ...
            content += '<div class="gwcone" id="CartShop_' + val.id + '" data-count="' + val.num + '" data-multi="' + val.multi + '">\n' +
                ...
                '                </div>\n' +
                '                <div class="go2"><a href="./?mod=buy4&cid=' + val.cid + '&tid=' + val.tid + '"><img src="' + val.shopimg + '" onerror="this.src=\'assets/store/picture/error_img.png\'"/></a></div>\n' +
                '                <div class="go3">\n' +
                '                    <div class="go3_1">\n' +
                '                        <a href="./?mod=buy4&cid=' + val.cid + '&tid=' + val.tid + '"><p class="p1">' + val.name + '</p></a>\n' +
                '                        <p class="p2" style="text-decoration: none"></p>\n' +
                '                    </div>\n' +
                '                    <div class="go3_2">\n' +
                '                        <p class="p3" style="font-size: 12px;">' + val.inputsdata + '</p>\n' +
                '                        <p class="p4" style="font-size: 14px;">￥' + val.money + '</p>\n' +
                '                    </div>\n' +
                '                    <div class="go3_3">\n' +
                '                        <div class="num1" data-id="' + val.id + '">-</div>\n' +
                '                        <div class="num2" id="Sum_' + val.id + '">' + val.num + '</div>\n' +
                '                        <div class="num3" data-id="' + val.id + '">+</div>\n' +
                '                        <div class="del" data-id="' + val.id + '" data-name="' + val.name + '"><img src="./assets/img/xtb/shanchu.png"/></div>\n' +
                '                    </div>\n' +
                '                </div>\n' +
                '            </div>';
        });
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.code == 0) {
					...
					setTimeout(function () {
						layer.alert('若未跳转付款界面,请点击下方付款按钮！', {
							title: '温馨提示',
							btn: ['付款','关闭'], time: 9999999, btn1: function (layero, index) {
								window.location.href = url;
							}
						})
					}, 500);
    ...
    PaySrtSucc: function (data) {
        ...
        $.each(data.data, function (key, val) {
            ...
            content += '<div ' + (window.innerWidth > 480 ? 'class="layui-col-sm' + (les == 1 ? '12' : '6') + '" style="margin-top: 0.7em;"' : 'class="layui-col-xs12" style="padding: 0.1em 0.8em 0 0.8em;margin-bottom: 0.9em"') + '>\n' +
                ...
                '                        购买数量：' + val.num + '份\n' +
                '                        <span color="#ff0000" style="float: right;font-size: 1.1em"><b>￥' + val.money + '</b></span>\n' +
                '                    </div>\n' +
