		...
		success : function(data) {
			...
			if(data.gift != null){
				$.each(data.gift, function(k, v) {
					$('#pst_1').append('<li><strong>某用户</strong> 抽中&nbsp;'+v+'</li>');
				});
				$('.giftlist').show();
	...
	if(inputsname!=''){
		$.each(inputsname.split('|'), function(i, value) {
			...
			if(value.indexOf('{')>0 && value.indexOf('}')>0){
				...
				$('#inputsname').append('<div class="form-group"><div class="input-group"><div class="input-group-addon" id="inputname'+(i+2)+'" gettype="'+inputsnametype+'">'+value+'</div><input type="number" name="inputvalue'+(i+2)+'" id="inputvalue'+(i+2)+'" value="1" class="form-control" required min="1" max="99999" onchange="getmulti(this)" act="getmulti"/></div></div>');
			}else if(inputsnametype=='domain'){
				$('#inputsname').append('<div class="form-group"><div class="input-group"><div class="input-group-addon" id="inputname'+(i+2)+'" gettype="'+inputsnametype+'">'+value+'</div><input type="text" name="inputvalue'+(i+2)+'" id="inputvalue'+(i+2)+'" value="" class="form-control" onchange="getDomain(\'inputvalue'+(i+2)+'\')" required/></div></div>');
			}else{
				...
				$.ajax({
					...
					success:function(data){
						if(data.code == 0){
							...
							layer.alert('恭喜您抽到奖品：'+data.name+'，请点击确定进入购买商品到付费界面即可看到相应的折扣价格。', {
