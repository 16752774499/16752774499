...
if(islogin!=1){
	var confirmobj = layer.confirm('你还未登录，是否现在登录？', {
	  btn: ['登录','注册','取消']
		...
		window.location.href='./user/login.php?back=seckill';
	}, function(){
		window.location.href='./user/reg.php?back=seckill';
	}, function(){
...
function getPoint() {
	...
	if(inputname=='hide'){
		...
		$('#inputsname').append('<div class="layui-form-item"><div class="input-group"><div class="input-group-addon" id="inputname">'+inputname+'</div><input type="text" name="inputvalue" id="inputvalue" value="'+($_GET['qq']?$_GET['qq']:'')+'" class="layui-input" required onblur="checkInput()"/></div></div>');
	}
	...
	if(inputsname!=''){
		$.each(inputsname.split('|'), function(i, value) {
			...
			if(value.indexOf('{')>0 && value.indexOf('}')>0){
				...
				$('#inputsname').append('<div class="layui-form-item"><div class="input-group"><div class="input-group-addon" id="inputname'+(i+2)+'">'+selectname+'</div><select name="inputvalue'+(i+2)+'" id="inputvalue'+(i+2)+'" class="layui-input">'+addstr+'</select></div></div>');
			}else{
			if(value=='说说ID'||value=='说说ＩＤ'||inputsnametype=='ssid')
				...
				var addstr='';
			$('#inputsname').append('<div class="layui-form-item"><div class="input-group"><div class="input-group-addon" id="inputname'+(i+2)+'" gettype="'+inputsnametype+'">'+value+'</div><input type="text" name="inputvalue'+(i+2)+'" id="inputvalue'+(i+2)+'" value="" class="layui-input" required/>'+addstr+'</div></div>');
			}
		});
	}
		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				if($('#show_shuoshuo').length > 0){
					$('#show_shuoshuo').html('<div class="input-group"><div class="input-group-addon onclick" title="上一页" onclick="get_shuoshuo(\''+id+'\',$(\'#inputvalue\').val(),'+km+','+lastpage+')"><i class="fa fa-chevron-left"></i></div><select id="shuoid" class="layui-input" onchange="set_shuoshuo(\''+id+'\');">'+addstr+'</select><div class="input-group-addon onclick" title="下一页" onclick="get_shuoshuo(\''+id+'\',$(\'#inputvalue\').val(),'+km+','+nextpage+')"><i class="fa fa-chevron-right"></i></div></div>');
				}else{
		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				if($('#show_rizhi').length > 0){
					$('#show_rizhi').html('<div class="input-group"><div class="input-group-addon onclick" onclick="get_rizhi(\''+id+'\',$(\'#inputvalue\').val(),'+km+','+lastpage+')"><i class="fa fa-chevron-left"></i></div><select id="blogid" class="layui-input" onchange="set_rizhi(\''+id+'\');">'+addstr+'</select><div class="input-group-addon onclick" onclick="get_rizhi(\''+id+'\',$(\'#inputvalue\').val(),'+km+','+nextpage+')"><i class="fa fa-chevron-right"></i></div></div>');
				}else{
...
function getCity(inputid,fid,i){
	...
	fid = fid || 0;
	if(i == 0){
		var options='<select class="layui-input" id="biaozhi_'+(i+1)+'" onchange="getCity(\''+inputid+'\',this.value,'+(i+1)+')">';
	...
	$.ajax({
		...
		success:function(data){
			if(data.length<1){
				...
				if($("#more_dizhi").length>0){}else $("#layer_button").append('<input class="layui-input" id="more_dizhi" placeholder="详细地址(村、门牌号)">');
				return false;
			}
			var options='<select class="layui-input" id="biaozhi_'+(i+1)+'" onchange="getCity(\''+inputid+'\',this.value,'+(i+1)+')">';
			options+='<option>请选择地址</option>';
	...
	function seckill_create(sid){
		var ii = layer.load(2, {shade:[0.1,'#fff']});
		$.ajax({
			type : "POST",
			url : "ajax.php?act=seckill_create",
			data : {sid:sid,hashsalt:hashsalt},
			dataType : 'json',
			success : function(data) {
				layer.close(ii);
				if (data.code === 0) {
	...
	function seckill_toshop(arr) {
			var ii = layer.load(2, {shade:[0.1,'#fff']});
			$.ajax({
				type : "GET",
				url : "ajax.php?act=gettool&tid="+arr.tid,
				dataType : 'json',
				success : function(data) {
					layer.close(ii);
					if(data.code == 0){
						...
						$('#toseckillshop').show();
						$('#seckillshop').hide();
						$('#name').val(res.name+'【'+arr.num+' 份】');
						$('#need').val('￥'+arr.price +"元");
						$("#tidframe").html('<select name="tid" class="layui-hide" id="tid"><option value="'+res.tid+'" sid="'+arr.sid+'" seckill_id="'+arr.seckill_id+'" cid="'+res.cid+'" price="'+arr.price+'" num="'+arr.num+'" desc="'+escape(res.desc)+'" alert="'+escape(res.alert)+'" inputname="'+res.input+'" inputsname="'+res.inputs+'" multi="'+res.multi+'" isfaka="'+res.isfaka+'" count="'+res.value+'" close="'+res.close+'" prices="'+res.prices+'" max="'+res.max+'" min="'+res.min+'">'+res.name+'</option></select>');
						getPoint();
					}else{
						layer.alert(data.msg);
					}
...
$(document).ready(function(){
	...
	$("#submit_buy").click(function(){
		var tid=$("#tid").val();
		if(tid==0 || $("#tid").length==0){layer.alert('请重试！');return false;}
		...
		$.ajax({
			type : "POST",
			url : "ajax.php?act=seckill_buy",
			data : {seckill_id:$('#tid option:selected').attr('seckill_id'),sid:$('#tid option:selected').attr('sid'),tid:tid,inputvalue:$("#inputvalue").val(),inputvalue2:$("#inputvalue2").val(),inputvalue3:$("#inputvalue3").val(),inputvalue4:$("#inputvalue4").val(),inputvalue5:$("#inputvalue5").val(),hashsalt:hashsalt},
			dataType : 'json',
			success : function(data) {
				layer.close(ii);
				if (data.code === 0) {
