...
while($res = $rs->fetch())
{
    if($res['cidr']>=1){$add=' <button class="label  label-success" type="button"> 上级id <span class="Danger">'.$res['cidr'].'</span></button>';}else{$add='';}
	echo '<tr data-cid='.$res['cid'].'>
	...
	</td><td><input type="text" class="form-control input-sm" name="name['.$res['cid'].']" value="'.$res['name'].'" placeholder="分类名称" required></td><td><span class="btn btn-primary btn-sm" onclick="editClass('.$res['cid'].')">修改</span>&nbsp;'.($res['active']==1?'<span class="btn btn-sm btn-success" onclick="setActive('.$res['cid'].',0)">显示</span>':'<span class="btn btn-sm btn-warning" onclick="setActive('.$res['cid'].',1)">隐藏</span>').'&nbsp;<a href="./shoplist.php?cid='.$res['cid'].'" class="btn btn-info btn-sm" target="_blank">商品</a>&nbsp;<span class="btn btn-sm btn-danger" onclick="delClass('.$res['cid'].')">删除</span>&nbsp;<div class="btn-group"><button type="button" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">更多 <span class="caret"></span></button><ul class="dropdown-menu dropdown-menu-right text-left"><li><a href="list.php?cid='.$res['cid'].'">查看该分类的订单</a></li><li><a href="javascript:setBlockPay('.$res['cid'].')">设置禁用支付方式</a></li>'.($conf['classblock']>0?'<li><a href="javascript:setClass('.$res['cid'].')">设置不可售地区</a></li>':null).'</ul></div> 
	<button class="label  label-success" type="button"> id <span class="Danger">'.$res['cid'].'</span></button>
   '.$add.'

	</td></tr>';
}
echo '<tr><td><span class="btn btn-primary btn-sm" onclick="saveAll()"><i class="fa fa-floppy-o"></i> 保存全部</span></td><td><input type="text" style="width:65%;float:left;" class="form-control input-sm" name="addname" placeholder="分类名称" required>
<input type="text" style="width:33%;float:right;"  class="form-control input-sm" name="cidr" placeholder="上级分类cid" required>
</td><td colspan="3"><span class="btn btn-success btn-sm" onclick="addClass()"><span class="glyphicon glyphicon-plus"></span> 添加分类</span>&nbsp;&nbsp;<a href="./classlist.php?my=classimg" class="btn btn-info btn-sm"><i class="fa fa-picture-o"></i> 修改分类图片</a></td></tr>';
?>
