...
<thead><tr><th>ID</th><th>奖品名称</th><th>对应商品</th><th>中奖几率</th><th>操作</th></tr></thead>
<tbody>
<?php
$rs = $DB->query("SELECT a.*,(SELECT b.name FROM pre_tools as b WHERE a.tid=b.tid) as shopname FROM pre_gift as a");

while ($row = $rs->fetch()) {
	echo '<tr><td>'.$row["id"].'</td><td>'.$row["name"].'</td><td>'.$row["shopname"].'</td><td>'.$row["rate"].'%</td><td><a href="javascript:void(0)" onclick="editmember(\'' . $row["id"] . '\')" class="btn btn-info btn-xs">编辑</a>&nbsp;<a href="javascript:void(0)" class="btn btn-xs btn-danger" onclick="del_member(\'' . $row["id"] . '\')">删除</a></td></tr>';
}
?>
</tbody>
</table>
