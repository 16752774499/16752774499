...
<tbody>
<tr align="center"><td>普通用户价格</td><td>分站站长价格</td><td>顶级合伙人价格</td></tr>
<tr>
...
break;
	...
	case 'setStock': //设置商品库存
		...
		break;
case 'updaim': //批量设置商品diam
  $daima=$_POST['dai'];
    $tid=$_POST['tid'];
	
			$DB->exec("update pre_tools set daim='$daima' where tid='{$tid}' and is_curl!=4");
...
case 'addPriceRule': //添加加价模板
	...
	if($name==null || $p_2==null || $p_1==null || $p_0==null){
		...
		exit('{"code":-1,"msg":"顶级合伙人加价不能高于分站站长加价"}');
	}elseif($p_2>$p_0){
		exit('{"code":-1,"msg":"顶级合伙人加价不能高于普通用户加价"}');
	}elseif($p_1>$p_0){
		exit('{"code":-1,"msg":"分站站长加价不能高于普通用户加价"}');
...
case 'getClassList': //获取对接分类列表
	...
	case 'getGoodsList': //获取对接商品列表
		...
		$row = $DB->getRow("select * from pre_shequ where id='$shequ' limit 1");
		$list = third_call($row['type'], $row, 'goods_list', [$classid]);
		if (!is_array($list)) $result =['code' => -1, 'msg' => $list];
		else $result = ['code' => 0, 'msg' => 'succ', 'type' => $row['type'], 'data' => $list];
		exit(json_encode($result));
		break;
	case 'getGoodsParam': //获取对接参数名
		$shequ = intval($_POST['shequ']);
	...
	case 'changePriceRule':
		adminpermission('price', 2);
		$id = intval($_POST['id']);
		if (!isset($_POST['cids'])) exit('{"code":-1,"msg":"请勾选分类！"}');
		...
		$sqls = trim($sqls, ',');
		$count = $DB->exec("UPDATE pre_tools SET prid='$id' WHERE cid IN ($sqls) AND price>0");
		if ($count !== false) {
			exit('{"code":0,"msg":"成功更改' . $count . '个商品的加价模板"}');
		} else {
			exit('{"code":-1,"msg":"更改失败！' . $DB->error() . '"}');
	...
	case 'getBatchGoodsList': //获取对接商品列表
		...
		$shequ = intval($_POST['shequ']);
		$row = $DB->getRow("select * from pre_shequ where id='$shequ' limit 1");
		if (!$row) exit('{"code":-1,"msg":"该社区ID不存在！"}');
	...
	case 'batch_merchandise':
		...
		$jump = intval($_POST['jump']);
		$goods_id = intval($_POST['goods_id']);
		$goodRow = $DB->getRow("select name from pre_tools where goods_id='$goods_id' limit 1");
		if ($jump == 1 && $goodRow) {
			exit(json_encode(['code' => 0, 'msg' => '已跳过此商品', 'name' => $goodRow['name']]));
		}
		$shequ = intval($_POST['shequ']);
		...
		if ($getinfo) {
			...
			if ($shequRow['type'] == 'zhike') {
				$goods_param = $goods_id . '#' . $getinfo['alias'];
			} else {
			...
			$sql = "INSERT INTO `pre_tools` (`cid`,`name`,`price`,`prid`,`input`,`inputs`,`desc`,`alert`,`shopimg`,`repeat`,`value`,`is_curl`,`shequ`,`goods_id`,`goods_type`,`goods_param`,`multi`,`min`,`max`,`sort`,`active`,`addtime`) VALUES ('" . $cid . "','" . $name . "','" . $price . "','" . $prid . "','" . $input . "','" . $inputs . "','" . addslashes($desc) . "','" . addslashes($alert) . "','" . $shopimg . "','" . $repeat . "','" . $value . "','2','" . $shequ . "','" . $goods_id . "','" . $goods_type . "','" . $goods_param . "','" . $multi . "','" . $min . "','" . $max . "','" . ($sort + 1) . "','1','"  . $date .  "')";
			if ($DB->exec($sql) !== false) {
				$result = ['code' => 0, 'msg' => '上架成功！', 'name' => $getinfo['name']];
			} else {
				$result = ['code' => -1, 'msg' => '上架失败！' . $DB->error() . '', 'name' => $getinfo['name']];
			}
		}
		exit(json_encode($result));
		break;
