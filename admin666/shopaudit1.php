...
$orderby = 'A.tid desc';
	$numrows=$DB->getColumn("SELECT count(*) from pre_tools WHERE proo>'0'");
	$sql=" proo>'0'";
...
<form onsubmit="return saveSetting(this)" method="post" class="form-horizontal form-bordered" role="form">
	  ...
	  <div class="col-sm-9"><input type="text" name="tctid" value="<?php echo $_GET['tc']?>" class="form-control" placeholder="提成金额"></div>
...
<div class="block">
<div class="block-title clearfix">
<h2>用户共上传平台 <b><?php echo $numrows ?></b> 个商品</h2></div>
	  <form name="form1" id="form1">
	  <div class="table-responsive">
        <table class="table table-striped table-bordered table-vcenter orderList">
          <thead><tr><th>商品名称</th><th>商品价格设置</th><th>所属用户zid</th>><th>操作</th><th>状态</th><th>微信</th><th>链接</th></tr></thead>
          <tbody>
<?php
...
echo '<tr '.($sup==1?'style="background-color:#454e59;"':'').'><td>
<a href="javascript:show('.$res['tid'].')" style="color:#000">'.$res['name'].'</a></td>
'.($res['prid']>0?'<td><span onclick="getPrice('.$res['tid'].')">


<font color="blue">'.$price_class[$res['prid']].'</font>&nbsp;(成本:'.$res['price'].')</span></td>':'<td>

<span onclick="getPrice('.$res['tid'].')">'.$res['price'].'｜'.$res['cost'].'｜'.$res['cost2'].'</span></td>').'
<td>


<a href="./userlist.php?my=edit&zid='.$res['proo'].'" style="color:#000">zid:'.$res['proo'].'</a></td>
...
<td>'.($res['close']==1?'
    ...
    <a class="btn btn-xs btn-success" href="?close='.$res['tid'].'">上架中</a>').''.$susu.'</td><td>
    
    <a href="./shopedit.php?my=edit&tid='.$res['tid'].'" class="btn btn-info btn-xs">编辑</a>&nbsp;<a href="?

tc='.$res['tid'].'" class="btn btn-xs btn-info">提成</a>&nbsp;<a href="?


sc='.$res['tid'].'" class="btn btn-xs btn-danger">删除</a></td>

<td><a href="./userlist.php?my=edit&zid='.$res['wx_tett'].'" style="color:#000">'.$res['wx_tett'].'</a></td>



<td><a href="./userlist.php?my=edit&zid='.$res['descc'].'" style="color:#000">'.$res['descc'].'</a></td><td>
