...
?>
	  <div class="table-responsive">
        <table class="table table-striped">
          <thead><tr><th>ID</th><th>商品名称</th><th>查单QQ</th><th>下单账号</th><th>key</th><th>奖励次数</th><th>进度</th><th>申请时间</th><th>状态</th><th>操作</th></tr></thead>
          <tbody>
<?php
...
echo '<tr><td><b>'.$res['id'].'</b></td><td><a href="./shoplist.php?tid='.$res['tid'].'">'.$res['name'].'</a></td><td>'.$res['qq'].'</td><td>'.$input_arr[0].'</td><td>'.$res['key'].'</td><td>'.$res['count'].'</td><td>'.$progress.'</td><td>'.$res['date'].'</td><td>'.$status.'</td><td><span class="btn btn-xs btn-danger" onclick="delInvite('.$res['id'].')">删除</span></td></tr>
';
}
?>
          </tbody>
