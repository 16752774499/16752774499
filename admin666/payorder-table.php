...
if(isset($_GET['column']) && isset($_GET['kw']) && !empty($_GET['kw'])){
	...
	$link=$links;
	$numrows=$DB->getColumn("SELECT count(*) FROM pre_pay WHERE channel IS NOT NULL AND{$sql}");
	$con='系统共有 <b>'.$numrows.'</b> 条支付记录<span class="text-muted">【<a href="record.php">查看收支明细</a>】<br>(未支付记录自动在24小时后删除)</span>';
}
?>
	  <div class="table-responsive">
        <table class="table table-striped table-bordered table-vcenter orderList">
          <thead><tr><th>状态</th><th>订单金额</th><th>创建时间<br/>完成时间</th><th>站点ID<br/>用户ID</th><th>支付订单号<br/>支付接口订单号</th><th>支付接口</th><th>订单名称</th><th>下单内容<br/>用户IP</th><th>商品ID</th><th>状态</th><th>操作</th></tr></thead>
          <tbody>
<?php
...
while($res = $rs->fetch())
{
	...
	echo '<tr><td>'.($res['status']==1?'<font color="green">已支付</font>':'<font color="red">未支付</font>').'</td><td>￥'.$res['money'].'</td><td>'.$res['addtime'].'<br/>'.$res['endtime'].'</td><td><a href ="sitelist.php?zid='.$res['zid'].'" target="_blank">'.$res['zid'].'</a><br/>'.(strlen($res['userid'])!=32?'<a href ="userlist.php?zid='.$res['userid'].'" target="_blank">'.$res['userid'].'</a>':'游客').'</td><td><b>'.$res['trade_no'].'</b><br/>'.$res['api_trade_no'].'</td><td>'.$typename[$res['type']].'<br/>('.$res['channel'].')</td><td>'.$res['name'].'</td><td>'.$res['input'].'<br/><a href="https://m.ip138.com/iplookup.asp?ip='.$res['ip'].'" target="_blank" rel="noreferrer">'.$res['ip'].'</a></td><td>'.($res['tid']>0?'<a href ="shoplist.php?tid='.$res['tid'].'" target="_blank">'.$res['tid'].'</a>':$res['tid']).'</td><td>'.($res['status']==1?'<font color="green">已支付</font>':'<font color="red">未支付</font>').'</td><td>'.($res['status']==1?'<a class="btn btn-xs btn-info" href="list.php?kw='.$res['trade_no'].'" target="_blank">订单</a>':'<a class="btn btn-xs btn-warning" href="javascript:fillOrder(\''.$res['trade_no'].'\')">补单</a>').' <a class="btn btn-xs btn-danger" href="javascript:delOrder(\''.$res['trade_no'].'\')">删除</a></td></tr>
';
}
?>
          </tbody>
