...
if(isset($_GET['kw']) && !empty($_GET['kw'])) {
	...
	$ondate3=$DB->getColumn("select count(*) from pre_orders A where status=3{$sqls}");
	$ondate4=$DB->getColumn("select count(*) from pre_orders A where status=0{$sqls}");
	$con='待处理的有 <b>'.$ondate4.'</b> 个，正在处理的有 <b>'.$ondate2.'</b> 个，异常的有 <b>'.$ondate3.'</b> 个';
	...
	$ondate3=$DB->getColumn("select count(*) from pre_orders A where status=3{$sqls}");
	$ondate4=$DB->getColumn("select count(*) from pre_orders A where status=0{$sqls}");
	$con='当前有 <b>'.$numrows.'</b> 个订单，待处理的有 <b>'.$ondate4.'</b> 个，正在处理的有 <b>'.$ondate2.'</b> 个，异常的有 <b>'.$ondate3.'</b> 个';
}
?>
...
<?php echo $con?>
        <table class="table table-striped table-bordered table-vcenter orderList">
          <thead><tr><th>订单ID</th><th>商品名称</th><th>下单数据</th><th>份数</th><th>站点ID</th><th>用户ID</th><th>支付金额</th><th>购买时间</th><th>对接状态</th><th>订单状态</th><th>查看</th><th>操作</th></tr></thead>
          <tbody>
<?php
...
while($res = $rs->fetch())
{
echo '<tr><td><input type="checkbox" name="checkbox[]" id="list1" value="'.$res['id'].'" onClick="unselectall1()"><b>'.$res['id'].'</b></td><td><span onclick="showOrder('.$res['id'].')" title="点击查看详情">'.$res['name'].'</span></td><td class="wbreak"><span onclick="inputOrder('.$res['id'].')" title="点击修改数据">'.$res['input'].($res['input2']?'<br/>'.$res['input2']:null).($res['input3']?'<br/>'.$res['input3']:null).($res['input4']?'<br/>'.$res['input4']:null).($res['input5']?'<br/>'.$res['input5']:null).'</span></td><td><span onclick="inputNum('.$res['id'].')" title="点击修改份数">'.$res['value'].'</span></td><td><a href ="sitelist.php?zid='.$res['zid'].'" target="_blank">'.$res['zid'].'</a></td><td>'.(strlen($res['userid'])!=32?'<a href ="userlist.php?zid='.$res['userid'].'" target="_blank">'.$res['userid'].'</a>':'0').'</td><td>'.$res['money'].'</td><td>'.$res['addtime'].'</td><td>'.display_djzt($res['djzt'],$res['id']).'</td><td>'.display_zt($res['status'],$res['id']).'</td><td><a href="./shoplist.php?&kw='.$res['name'].'">查看商品</a></td><td><select onChange="javascript:setStatus(\''.$res['id'].'\',this.value)" class="form-control"><option selected>操作订单</option><option value="0">待处理</option><option value="2">正在处理</option><option value="1">已完成</option><option value="4">已退单</option><option value="3">异常</option>'.($res['zid']>1||is_numeric($res['userid'])?'<option value="6">退款</option>':null).'<option value="5">删除订单</option></select></td></tr>';
}
?>
          </tbody>
        </table>
