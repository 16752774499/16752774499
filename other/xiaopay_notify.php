...
$url="http://pay.xiaomfzs.com/api/order/query";
$merchantId='7430';
$outOrderId=$_GET['trade_no'];
$key='ef2f6630a88e455ba1adb0928fa0a453';
$arr="inputCharset=utf-8&merchantId=$merchantId&outOrderId=$outOrderId&signType=MD5";
$signature=md5($arr.$key);
$postData=array('merchantId'=>$merchantId,'outOrderId'=>$outOrderId,'inputCharset'=>'utf-8','signType'=>'MD5','signature'=>$signature);
$data=curlPost($url,$postData);

$res=json_decode($data,true);

if($res['orderState']==7 and $outOrderId=$res['outOrderId']){
    $out_trade_no=$res['outOrderId'];
	...
	if ($srow['tid'] == -2) {
		...
		if ($srow['zid'] > 1) {
			...
			if ($siterow == 2 && $kind == 1) {
				...
				fx($money,$srow['zid']);
				changeUserMoney($srow['zid'], $money*0.9, true, "提成", "你网站的用户开通分站站长获得" . $money*0.9 . "元提成");
			} else {
				if ($kind == 1 && $conf['fenzhan_cost'] > 0 && $srow['money'] > $conf['fenzhan_cost']) {
					$money = round($srow['money'] - $conf['fenzhan_cost'], 2);
					fx($money,$srow['zid']);
					changeUserMoney($srow['zid'], $money*0.9, true, "提成", "你网站的用户开通分站获得" . $money*0.9 . "元提成");
				} else {
					if ($kind == 2 && $conf['fenzhan_cost2'] > 0 && $srow['money'] > $conf['fenzhan_cost2']) {
						$money = round($srow['money'] - $conf['fenzhan_cost2'], 2);
						fx($money,$srow['zid']);
						changeUserMoney($srow['zid'], $money*0.9, true, "提成", "你网站的用户开通顶级合伙人获得" . $money*0.9 . "元提成");
